...
class FORMATTER_TYPE(Enum):
    GENERALFORMATTER = 1    #‘’%(levelname)s - %(asctime)s - %(filename)s:%(lineno)d -[%(module)s:%(funcName)s] - %(message)s'
    NONEFORMATTER = 2   #‘%(message)s'
...
class RecordLog:
    ...
    def output_logging(self,FormatterType: FORMATTER_TYPE):
        loggerName = __name__
        strFormatter = '%(levelname)s - %(asctime)s - %(filename)s:%(lineno)d -[%(module)s:%(funcName)s] - %(message)s'

        if FormatterType == FORMATTER_TYPE.NONEFORMATTER:
            loggerName = "NONEFORMATTER"
            strFormatter = '%(message)s'
        ...
        if not logger.handlers:
            ...
            log_format = logging.Formatter(strFormatter)
            # 日志输出到指定文件，滚动备份日志
...
apilog = RecordLog()
logs = apilog.output_logging(FORMATTER_TYPE.GENERALFORMATTER)
logsNoFormartter = apilog.output_logging(FORMATTER_TYPE.NONEFORMATTER)